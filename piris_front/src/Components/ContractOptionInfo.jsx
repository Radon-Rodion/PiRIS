...
const ContractOptionInfo = ({ info, isDebet=true }) => {
    return (
        <div className='row'>
            ...
            <div className='col'>
                <p className='text row'>Сумма от:&nbsp;<span>{info.sunFrom} до {info.sumTo}</span>&nbsp;{info.availableCurrencies}</p>
                <p className='text row'>Длительность от:&nbsp;<span>{info.sunFrom} до {info.sumTo} мес.</span></p>
                ...
                <p className='text row'>Процент в год:&nbsp;<span>{info.percentPerYear}</span></p>
            </div>
        </div>
    );
}
